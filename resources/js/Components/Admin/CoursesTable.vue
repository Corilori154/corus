...
<template>
    <section class="overflow-hidden rounded-3xl bg-white">
        <div class="border-b border-black/5 px-6 py-5">
            <h2 class="font-serif text-2xl">Tous les cours</h2>
            <p class="mt-1 text-xs text-black/45">{{ courses.length }} cours configurés</p>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1100px] text-left">
                <thead class="bg-[#f7f5f0] text-[11px] font-bold uppercase tracking-wider text-black/45">
                    <tr><th class="px-5 py-4">Cours</th><th class="px-5 py-4">Saison</th><th class="px-5 py-4">Horaire et lieu</th><th class="px-5 py-4">Capacité</th><th class="px-5 py-4">Tarifs</th><th class="px-5 py-4">Statut</th><th class="px-5 py-4 text-right">Actions</th></tr>
                </thead>
                <tbody class="divide-y divide-black/5">
                    <template v-for="course in courses" :key="course.id">
                        <tr class="cursor-pointer align-middle hover:bg-[#faf9f6]" @click="router.visit(`/admin/cours/${course.id}`)">
                            <td class="px-5 py-5"><div class="flex items-center gap-3"><img :src="course.image" :alt="course.title" class="h-14 w-16 rounded-xl object-cover" /><div><strong>{{ course.title }}</strong><p class="mt-1 text-xs text-black/40">{{ course.style }} · {{ course.level }} · {{ course.teacher }}</p><span v-if="course.couple_mode" class="mt-1 inline-block text-[10px] font-bold uppercase text-purple-700">Lead/Follow · écart {{ course.max_role_gap }}</span></div></div></td>
        ...
        <div v-if="!courses.length" class="px-6 py-16 text-center text-black/40">Aucun cours.</div>
    </section>
</template>
