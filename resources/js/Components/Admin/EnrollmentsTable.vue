            ...
            <table class="w-full min-w-[1050px] text-left">
                ...
                <tbody class="divide-y divide-black/5">
                    <tr v-for="item in enrollments" :key="item.id" class="hover:bg-[#faf9f6]">
                        <td class="px-6 py-5"><strong>{{ item.first_name }} {{ item.last_name }}</strong><a :href="`mailto:${item.email}`" class="mt-1 block text-xs text-black/40">{{ item.email }}</a><a v-if="item.phone" :href="`tel:${item.phone}`" class="text-xs text-black/40">{{ item.phone }}</a><p v-if="item.comment" class="mt-2 max-w-xs whitespace-pre-wrap rounded-lg bg-[#f7f5f0] p-2 text-xs text-black/60">{{ item.comment }}</p></td>
