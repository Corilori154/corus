            ...
            <section class="mt-8 overflow-hidden rounded-3xl bg-white">
                ...
                <div v-if="enrollments.length" class="divide-y divide-black/5">
                    <article v-for="item in enrollments" :key="item.id" class="grid gap-4 p-6 md:grid-cols-[1fr_auto] md:items-center">
                        <div class="flex items-center gap-4"><img v-if="item.course?.image" :src="item.course.image" :alt="item.course.title" class="h-16 w-16 rounded-2xl object-cover" /><div><h3 class="font-serif text-xl">{{ item.course?.title }}</h3><p class="mt-1 text-sm text-black/45">{{ item.course?.day }} · {{ item.course?.time }} · {{ item.course?.location }}</p><p class="mt-1 text-xs text-black/40">Début le {{ date(item.start_date) }}</p></div></div>
